...
Triple <List <std::pair <Student, float>>, List <std::pair <Student, float>>, List <std::pair <Student, float>>> University::split (const std::string group, const unsigned short session, const std::string gender) {
    List <std::pair <Student, float>> students;
    for (int i = 0; i < this->database.length(); ++i) {
        if ((this->database[i].getGroup() == group) && (this->database[i].getGender() == gender)) {
            ...
            if ((session <= unit.getNumberOfSessions()) && (session > 0)) {
                ...
                students.append(std::make_pair(unit, avg));
            }
        }
    }
    Triple <List <std::pair <Student, float>>, List <std::pair <Student, float>>, List <std::pair <Student, float>>> result;
    for (int i = 0; i < students.length(); ++i) {
        if (students[i].second >= 4.5) { 
            result.first.append(std::make_pair(students[i].first, students[i].second)); 
        }
        else if (students[i].second >= 3.5) {
            result.second.append(std::make_pair(students[i].first, students[i].second));
        }
        else if (students[i].second >= 2.5) {
            result.third.append(std::make_pair(students[i].first, students[i].second));
        }
    }
    return result;
}

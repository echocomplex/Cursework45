...
void sortGroup (University& unit) {
    ...
    std::cin >> gender;
    Triple <List <std::pair <Student, float>>, List <std::pair <Student, float>>, List <std::pair <Student, float>>> result = unit.split(group, session, gender);
    if ((result.first.length() == 0) && (result.second.length() == 0) && (result.third.length() == 0)) {
        throw std::logic_error("Group doesn't exist or gender is wrong!");
    }
